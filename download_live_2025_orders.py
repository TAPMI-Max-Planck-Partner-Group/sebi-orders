...
MD_FILE = "/Users/soveet/.gemini/antigravity-ide/brain/e050fa4b-37d3-4174-8636-bfd64ca0080a/live_downloads_2025.md"
...
def download_sebi_orders(dest_dir="sebi order 2025"):
    # Clear dest_dir
    ...
    with sync_playwright() as p:
        ...
        while not reached_2024:
            ...
            for date_text, title, url in items:
                if "2024" in date_text and "2025" not in date_text and "2026" not in date_text:
                    reached_2024 = True
                    break
                    
                is_2025 = "2025" in date_text
                
                if not is_2025:
                    continue
                
                processed_count += 1
                print(f"[{processed_count}] Date: {date_text} | 2025?: True")
                ...
                if url.lower().endswith(".pdf"):
                    ...
                    try:
                        # Open in a new page/tab so we don't break pagination history
        ...
        browser.close()
        print(f"\\nDone. Processed {processed_count} orders for 2025.")
        
        # Append completion status
        with open(MD_FILE, "a", encoding="utf-8") as f:
            f.write(f"\n\n**Finished Downloading {processed_count} SEBI orders for 2025.**\n")
